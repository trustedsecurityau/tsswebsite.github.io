<template>
    <div class="operational">
        <header-view theme="red"
                     top-bar-colour="#ea4141"/>

        <ServiceHero :service-type="Services.securitytesting"/>

        <ServiceFeatured v-for="service in Services.securitytesting.list"
        ...
        <ServiceRow
            v-for="service in Services.securitytesting.list"
            v-if="service.type === 'row'"
            :service="service"
            :key="service.id" />

        <ServiceSectionFeaturettes :service-list="Services.securitytesting.list"
                                   skrim/>

        <div class="grid-x grid-padding-x">
            <careers-router class="medium-auto small-12 cell"/>
            <consulting-router class="medium-auto small-12 cell"/>
        </div>
        <footer-view/>
    </div>
</template>
...
import ConsultingRouter from '@/components/modules/routers/Consulting'
...
<style lang="scss" scoped>
    ...
    /deep/ {
        ...
        section#hero {
            .main {
                background-image: url('/img/services/operational/operational_main_2x.png');
            }
            .sub {
                background-image: url('/img/services/operational/operational_sub_2x.png');
            }

            p {
                opacity: 0.9;
                color: lighten($operational-color, 15%);
            }
        }
        section.feature {
            h2 {
